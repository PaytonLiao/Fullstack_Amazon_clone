import { Product } from "./types/Product"

export const sampleProducts: Product[] = [
  {
    name: "Nike Slim shirt",
    slug: "nike-slim-shirt",
    category: "Shirts",
    image: "../images/p1.jpg",
    price: 120,
    countInStock: 10,
    brand: "Nike",
    rating: 4.5,
    numReviews: 10,
    description: "high quality shirt",
  },
  {
    name: "Adidas Fit Shirt",
    slug: "adidas-fit-shirt",
    category: "Shirts",
    image: "../images/p2.jpg",
    price: 100,
    countInStock: 20,
    brand: "Adidas",
    rating: 4.0,
    numReviews: 10,
    description: "high quality product",
  },
  {
    name: "Lacoste Free Pants",
    slug: "lacoste-free-pants",
    category: "Pants",
    image: "../images/p3.jpg",
    price: 220,
    countInStock: 0,
    brand: "Lacoste",
    rating: 4.8,
    numReviews: 17,
    description: "high quality product",
  },
  {
    name: "Nike Slim Pant",
    slug: "nike-slim-pant",
    category: "Pants",
    image: "../images/p4.jpg",
    price: 78,
    countInStock: 15,
    brand: "Nike",
    rating: 4.5,
    numReviews: 14,
    description: "high quality product",
  },
  {
    name: "Adidas Track Pants",
    slug: "adidas-track-pants",
    category: "Pants",
    image: "../images/p5.jpg",
    price: 85,
    countInStock: 20,
    brand: "Adidas",
    rating: 4.7,
    numReviews: 23,
    description: "high quality product",
  },
  {
    name: "Puma Joggers",
    slug: "puma-joggers",
    category: "Pants",
    image: "../images/p6.jpg",
    price: 65,
    countInStock: 10,
    brand: "Puma",
    rating: 4.3,
    numReviews: 11,
    description: "comfortable and stylish joggers",
  },
  {
    name: "Under Armour Sweatpants",
    slug: "under-armour-sweatpants",
    category: "Pants",
    image: "../images/p7.jpg",
    price: 95,
    countInStock: 8,
    brand: "Under Armour",
    rating: 4.6,
    numReviews: 19,
    description: "premium sweatpants for everyday wear",
  },
  {
    name: "Reebok Training Pants",
    slug: "reebok-training-pants",
    category: "Pants",
    image: "../images/p8.jpg",
    price: 70,
    countInStock: 18,
    brand: "Reebok",
    rating: 4.4,
    numReviews: 16,
    description: "perfect for workouts and casual wear",
  },
  {
    name: "Levi's Casual Pants",
    slug: "levis-casual-pants",
    category: "Pants",
    image: "../images/p9.jpg",
    price: 120,
    countInStock: 12,
    brand: "Levi's",
    rating: 4.7,
    numReviews: 20,
    description: "stylish and durable casual pants",
  },
]
